...
# Optional gaze tracking - falls back to simple tracking if dlib not available
try:
    ...
    print(f"Gaze tracking not available: {e}. Eye movement detection will be limited.")
...
try:
    ...
    print(f"Landmark model not available: {e}. Head pose detection will be limited.")
    landmark_model = None
    LANDMARK_AVAILABLE = False
    
def get_frame(imgData):
    nparr = np.frombuffer(base64.b64decode(imgData), np.uint8)
    image = cv2.imdecode(nparr, cv2.IMREAD_COLOR)  # Fixed: full color decode
    ret = True
    ...
    count = 0
    mob_status = 0   # Default: no phone
    for i in range(nums[0]):
        cls_id = int(classes[0][i])
        if cls_id == 0:          # class 0 = person
            count += 1
        if cls_id == 67:         # class 67 = cell phone (COCO)
            print('Mobile Phone detected')
            mob_status = 1       # Only set to 1, never reset back to 0 in loop

    if count == 0:
        print('No person detected')
        person_status = 0        # 0 = absent (no one in frame)
    elif count > 1:
        print('More than one person detected')
        person_status = 2        # 2 = multiple people
    else:
        print('Normal')
        person_status = 1        # 1 = exactly one person (normal)
    ...
    if ret == True and LANDMARK_AVAILABLE and landmark_model is not None:
        for face in faces:
    ...
    # Eye movement detection (with fallback if dlib not available)
    eye_movements = 0  # Default: not found

    if GAZE_AVAILABLE and gaze_tracker is not None:
        try:
            gaze_tracker.refresh(image)
        ...
        # Simple fallback: use face landmarks to estimate eye state
        # This is a basic implementation when dlib is not available
        if len(faces) > 0:
            # Mark that eyes were detected (but direction unknown)
            eye_movements = 2  # Assume center/valid
            print("Eye detection (fallback): eyes found")
        else:
            eye_movements = 0
            print("Eye detection (fallback): no face")
